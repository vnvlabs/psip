...
function toggle_psip(fileId) {
    if (!psip_readonly) {
        $.post("{{url_for('base.psip.toggle')}}?fileId=" + fileId, function(data) {
            if (data === "show") {
...
function save_state() {
    if (!psip_readonly) {
        url = "{{url_for('base.psip.save')}}?fileId=" + psip_file_id + "&data=" + encodeURIComponent(JSON.stringify(main_data))
